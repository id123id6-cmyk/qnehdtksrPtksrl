...
// 사이트 공유 URL (배포 도메인)
const SITE_URL = "https://seungbak.com/tools/apt-calculator";
...
function buildSalaryCalculatorUrl(priceMan, options = {}) {
  ...
  const debt = options.debtMan ?? getAmountValue("existingDebt") ?? "";
  const loanYears =
  ...
  params.set("price", String(priceMan));
  params.set("region", region);
  params.set("house", homeStatus);
  if (cash) params.set("cash", String(cash));
  if (debt) params.set("debt", String(debt));
  if (loanYears) params.set("years", String(loanYears));
  if (baseRate) params.set("rate", String(baseRate));
  if (options.fromForward !== false) params.set("from", "forward");

  return `../salary-calculator/?${params.toString()}`;
}

// 교정 박스·프리셋 → 연봉 계산기 페이지로 이동
function jumpToReverseTab(priceMan, options = {}) {
  ...
  window.location.href = buildSalaryCalculatorUrl(priceMan, options);
}
...
function handleReverseDeepLink() {
  ...
  if (params.get("mode") !== "reverse" && !params.get("price")) return;

  const priceMan = parseInt(params.get("price"), 10);
  if (!priceMan || priceMan < 1000) return;

  const query = params.toString();
  window.location.replace(`../salary-calculator/?${query}`);
}
